...
function getFileIconData(name) {
  ...
  if (img.includes(ext))
    return { icon: ImageIcon, bg: "bg-indigo-50", text: "text-indigo-600" };
  if (doc.includes(ext))
    return { icon: FileText, bg: "bg-emerald-50", text: "text-emerald-600" };
  if (vid.includes(ext))
    return { icon: Film, bg: "bg-rose-50", text: "text-rose-600" };
  if (aud.includes(ext))
    return { icon: Music, bg: "bg-amber-50", text: "text-amber-600" };
  if (arc.includes(ext))
    return { icon: Archive, bg: "bg-slate-100", text: "text-slate-600" };
  if (code.includes(ext))
    return { icon: Code2, bg: "bg-blue-50", text: "text-blue-600" };
...
function Modal({ open, onClose, title, children }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm transition-all duration-300 animate-in fade-in"
      onClick={onClose}
    >
      <div
              ...
              <div
                className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
                  isComplete
                    ...
                    : "bg-blue-100 text-brand animate-pulse"
                }`}
              >
                {isComplete ? (
...
function FileCard({
  ...
  return (
    <div className="group relative flex items-center justify-between border-b border-slate-100 last:border-0 bg-white p-3 hover:bg-slate-50 transition-colors">
      <div className="flex items-center gap-4 flex-1 min-w-0 px-2">
        <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center shadow-sm ${iconData.bg} ${iconData.text}`}>
          <Icon className="w-5 h-5" strokeWidth={2} />
        </div>
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-bold text-slate-900 truncate text-sm" title={f.file_name}>
              {f.file_name}
            </p>
            {/* Mobile badges inline with name */}
            <div className="md:hidden flex gap-1">
              {f.share_token && <Link2 className="w-3.5 h-3.5 text-cyan-500" />}
            </div>
          </div>
          <div className="flex items-center gap-2 text-xs font-medium text-slate-500 md:hidden mt-0.5">
            <span className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600">{formatSize(f.size)}</span>
            <span>•</span>
      ...
      <div className="hidden md:flex items-center gap-8 w-64 shrink-0 text-sm font-medium text-slate-600">
        <span className="w-20">{formatSize(f.size)}</span>
        <span className="flex-1 text-slate-500">{f.uploaded_at ? timeAgo(f.uploaded_at) : "—"}</span>
      </div>

      <div className="flex items-center justify-end gap-1 shrink-0 ml-4 w-32 px-2">
         {/* Desktop Badges */}
         {f.share_token && (
            <div className="hidden md:flex items-center justify-center w-7 h-7 rounded-full bg-cyan-50 text-cyan-600 mr-1" title="Shared via Link">
              <Link2 className="w-3.5 h-3.5" />
            </div>
          )}
          <button
            onClick={() => onToggleFavorite(f.file_id)}
          ...
          <div className="relative" ref={menuRef}>
            ...
            {/* Dropdown Menu */}
            <div
              className={`absolute right-0 top-full mt-2 w-48 rounded-2xl bg-white p-2 shadow-xl shadow-slate-200/50 border border-slate-100 transition-all duration-200 z-50 origin-top-right ${
                menuOpen
            ...
            >
              <button
                onClick={() => {
                  onShare(f.file_id, f.file_name);
                  setMenuOpen(false);
                }}
                className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-600 hover:bg-brand/5 hover:text-brand transition-colors"
              >
              ...
              <button
                onClick={() => {
                  onDownload(f.file_id, f.file_name);
                  setMenuOpen(false);
                }}
                className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-600 hover:bg-brand/5 hover:text-brand transition-colors"
              >
...
export function MyFiles() {
  ...
  const startUpload = useCallback(
    (file) => {
      ...
      if (file.size > CHUNK_THRESHOLD) {
        ...
        const { xhr, promise } = uploadFileWithProgress(
          file,
          activeFolder,
          onProgress
        );
  ...
  const handleToggleFavorite = async (fileId) => {
    try {
      const res = await apiFetch(`/files/${fileId}/favorite`, {
        method: "PATCH",
      });
      setFiles((prev) =>
        prev.map((f) =>
          f.file_id === fileId ? { ...f, is_favorite: res.is_favorite } : f
        )
      );
    } catch (err) {
  ...
  const handleShare = async (fileId, fileName) => {
    try {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.file_id === fileId ? { ...f, share_token: res.share_token } : f
        )
      );
    } catch (err) {
  ...
  const handleDeleteFolder = async (folderId) => {
    if (
      !window.confirm("Delete this folder? Files inside will be moved to root.")
    )
      return;
    try {
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
        ...
        <div className="md:col-span-2 relative overflow-hidden rounded-[2rem] bg-white border border-slate-100 shadow-sm p-8 flex flex-col justify-center">
          ...
          <div className="relative z-10">
            ...
            <p className="text-base text-slate-500 max-w-xl">
              Securely store, organize, and share all your documents across your
              workspace.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            {folders.map((fo) => (
              ...
              >
                ...
                >
                  <FolderOpen
                    className={`w-4 h-4 ${
                      activeFolder === fo.folder_id
                        ? "opacity-100"
                        : "opacity-70 text-brand"
                    }`}
                    fill={
                      activeFolder === fo.folder_id ? "currentColor" : "none"
                    }
                  />
                  {fo.name}
                </button>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDeleteFolder(fo.folder_id);
                  }}
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-r from-brand/10 to-indigo-500/10 opacity-0 transition-opacity duration-500 pointer-events-none ${
              dragOver ? "opacity-100 animate-pulse" : ""
            }`}
          />
          <div
            className={`h-full w-full rounded-[1.5rem] border-2 border-dashed flex flex-col items-center justify-center text-center p-6 transition-all duration-300 ${
              dragOver
      ...
      <div className="rounded-[2rem] border border-slate-100 bg-white shadow-sm flex flex-col">
        <div className="p-5 border-b border-slate-50 flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-slate-50/50 rounded-t-[2rem]">
          {/* Smart Search */}
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="flex flex-col border border-slate-100 rounded-2xl overflow-visible bg-white shadow-sm">
              {/* List Header */}
              <div className="hidden md:flex items-center justify-between border-b border-slate-100 bg-slate-50/80 p-3 text-xs font-bold text-slate-500 uppercase tracking-wider rounded-t-2xl">
      ...
      <Modal
        ...
        onClose={() => setShareModal({ open: false, url: "", fileName: "" })}
      ...
      >
        <div className="flex flex-col items-center justify-center py-4">
          ...
          <p className="text-sm font-bold text-slate-900 tracking-tight text-center truncate w-full px-4">
            {shareModal.fileName}
          </p>
          ...
          <p className="mt-4 text-xs font-medium text-slate-400 text-center">
            Share this link directly — no account required to download.
          </p>
        </div>
      </Modal>
      ...
      <Modal
        open={moveModal.open}
        onClose={() => setMoveModal({ open: false, fileId: null })}
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
          ...
          >
            <div className="flex items-center gap-3">
              <div className="flex justify-center items-center h-10 w-10 bg-slate-100 text-slate-600 rounded-lg">
          ...
          {folders.map((fo) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex justify-center items-center h-10 w-10 bg-brand/10 text-brand rounded-lg">
